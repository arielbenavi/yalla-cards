...
| `notes` | לא | הערות בעברית: הקשר שימוש, הבדל מילולי/יומיומי וכו׳ — **ללא** צורת רבים (ראה מטה) |

## כללים

- כל שדה שעוטף `**bold**` — הסקריפט מסיר את הכוכביות אוטומטית.
- **dedup**: השוואה לפי `translit_nikud` ללא ניקוד — שורה שכבר קיימת ב-DB תדולג.
- **צורת רבים**: כתוב בעמודת `notes` בפורמט `רבים: X (תרגום)` — הסקריפט מחלץ אוטומטית לשדה `plural_form` ומנקה מה-notes. **אל תמציא צורות רבים** — רק אם מופיעות בחומר.
  - דוגמה: `רבים: מַדַארֶס (בתי ספר)`
  - אפשר להוסיף הערה לפני: `שדה על ד. רבים: מַדַארֶס (בתי ספר)`
- **notes** (אחרי חילוץ הרבים): כתוב תמיד בעברית. דוגמאות:
  - `שימוש לגבר בלבד`
  - `מילולית: שלום לפניך — בפועל: שלום`
  - `שדה על ב` (הערה על שינוי אות בדגש)
